...
$ambilData = mysqli_fetch_array(mysqli_query($kon, "SELECT * FROM jurusan WHERE idJurusan = '$id'"));
$jurusan = $ambilData['namaJurusan'];
$idFakultas = $ambilData['idFakultas'];

// menampilkan seluruh data fakultas
$query = mysqli_query($kon, "SELECT * FROM fakultas");
while ($comboBox = mysqli_fetch_array($query)) {
  if ($idFakultas == $comboBox['idFakultas']) {
    $pil .= "
      <option value='{$comboBox['idFakultas']}' selected>{$comboBox['namaFakultas']}</option>
    ";
  } else {
    $pil .= "
...
$tombol = $_POST['tombol'];
if ($tombol) {
  $h = mysqli_query($kon, "UPDATE jurusan SET namaJurusan = '$jrsn', idFakultas = '$fakultas' WHERE idJurusan = '$id'");
  if ($h) {
    echo "
      <script>
    ...
    <div class="card">
      <div class="card-body">
        ...
        <form method="POST" action="">
          ...
          <input type="text" name="jurusan" id="jurusan" class="form-control" value="<?= $jurusan; ?>">
