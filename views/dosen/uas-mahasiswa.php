...
$h = $ambilData['idDosen'];
$query = mysqli_query($kon, "SELECT * FROM nilaiUas, mataKuliah, mahasiswa, kelas WHERE nilaiUas.idMatkul = mataKuliah.idMatkul AND nilaiUas.idMahasiswa = mahasiswa.idMahasiswa AND kelas.idKelas = '$id' AND nilaiUas.idDosen = '$h'");
if (mysqli_num_rows($query) > 0) {
  $no = 0;
  while ($row = mysqli_fetch_array($query)) {
    $no++;
    $UAS = mysqli_fetch_array(mysqli_query($kon, "SELECT * FROM nilaiUas WHERE idMatkul = '{$row['idMatkul']}'"));
    if ($UAS['nilai'] > 75) {
      $tbl = "-";
      $uas = $UAS['nilai'];
      $ket = $UAS['keterangan'];
    } else if ($UAS['nilai'] <= 75) {
      $tbl = "<a href='edit-uas.php?id={$row['idMatkul']}' class='btn'>Edit Nilai UAS</a>";
      $ket = $UAS['keterangan'];
    }
    $tr.= "
            <tr>
  ...
  <div class="container top-28">
    ...
    <div class="card">
      <div class="card-body">
        ...
        <p>Inpu Nilai UAS Mahasiswa Kelas : <?= $namaKelas; ?></p>
      </div>
    </div>
    <!-- akhir card -->
    ...
    <div class="card">
      <div class="card-body">
        <div class="row">
          ...
          <a href="uas.php?id=<?= $id; ?>" class="btn bottom">UAS</a>
        </div>
